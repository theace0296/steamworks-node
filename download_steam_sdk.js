...
const downloadSteamworksSdk = async () => {
  ...
  if (loginResponse.status !== 200) {
    throw new Error(
      `Failed to get Steam SDK: [${loginResponse.status}]: ${loginResponse.statusText}`,
    );
  }
  ...
  const hasSteamGuard =
    prompt('Do you use Steam Guard (2FA)? [y/N]: ', 'N').toLowerCase() === 'y';
  ...
  const sessionid = loginResponse.headers
    .raw()['set-cookie'].find(cookie => cookie.includes('sessionid'))
    .split(';')[0]
    .split('=')[1];
  const sessionCookie = loginResponse.headers
    .raw()['set-cookie'].map(cookie => cookie.split(';')[0])
    .join(';');
  ...
  if (
    rsaKeyResponse.status !== 200 ||
    !rsaKeyResponseData.success ||
    !rsaKeyResponseData.publickey_mod ||
    !rsaKeyResponseData.publickey_exp ||
    !rsaKeyResponseData.timestamp
  ) {
    throw new Error(
      'An error occured while trying to communicate with the Steam servers!',
    );
  }
  ...
  if (
    doLoginResponse.status !== 200 ||
    !doLoginResponseData.success ||
    !doLoginResponseData.login_complete
  ) {
    throw new Error('An error occured while authenticating with Steam!');
  }

  const loginCookie = `${sessionCookie};${doLoginResponse.headers
    .raw()['set-cookie'].map(cookie => cookie.split(';')[0])
    .join(';')}`;
  ...
  if (loginResponse.status !== 200) {
    throw new Error(
      `Failed to download Steam SDK: [${loginResponse.status}]: ${loginResponse.statusText}`,
    );
  }
  if (!sdkResponse.headers.raw()['content-type']?.includes('application/zip')) {
    throw new Error(
      'An unknown error occured while attempting to download the Steam SDK!',
    );
  }
