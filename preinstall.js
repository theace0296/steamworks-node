...
try {
    ...
    const getJsType = typeStr => {
      ...
      /char \[\d+\]/.test(typeStr)
        ...
        if (
            steamApiEnumNames.includes(typeStr) ||
      steamApiStructNames.includes(typeStr)
        ) {
            return typeStr;
        }
        return 'unknown';
    ...
    for (const steamStruct of steamApiJson.structs) {
      ...
      Array.isArray(steamStruct.consts)
            ...
            for (const constant of steamStruct.consts) {
                steamApiStructStr = `${steamApiStructStr}\n  ${
                    constant.constname
                }: ${getJsType(constant.consttype)} = ${constant.constval}${
                    isClass ? ';' : ','
                }`;
            }
        }
  ...
  `;
    ...
    for (const callResultFunction of Object.keys(callResultFunctionsMade)) {
      ...
      callResultFunctionsMade[callResultFunction];
        callResultFunctionsStr = `${callResultFunctionsStr}\nexport async function ${name}(${
            args.length ? args.map(arg => `${arg.name}: ${arg.type}`).join(', ') : ''
        }): Promise<{ ${Object.entries(returnType)
            .map(r => `${r[0]}: ${r[1]}`)
            .join(', ')} }>;`;
    }
