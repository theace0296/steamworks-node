...
try {
  ...
  for (const steamHeader of fs.readdirSync('./steam')) {
    if (fs.lstatSync(path.resolve('./steam', steamHeader)).isFile()) {
      ...
      const callbackMatches = content
        ...
        ?.map(match => /STEAM_CALLBACK_BEGIN\(\s*(\w+),\s*[^)]*\)/.exec(match)[1])
        ?.filter(match => match.toUpperCase() !== match);
      ...
      const interfaceMatches = content
        ...
        ?.filter(match => match.toUpperCase() !== match);
      if (interfaceMatches?.length && !['steam_api_internal.h', 'isteamps3overlayrenderer.h'].includes(steamHeader)) {
  ...
  for (const { name: steamInterface, file: interfaceHeaderFile } of SteamInterfaces) {
...
#define ${steamInterface.slice(1).toUpperCase()}_INTERFACE_VERSION "${steamInterface.slice(1)}001"
#endif
STEAM_DEFINE_USER_INTERFACE_ACCESSOR( ${steamInterface} *, ${steamInterface.slice(1)}, ${steamInterface.slice(1).toUpperCase()}_INTERFACE_VERSION );
`;
    }
  }
  steamInterfacesStr = `
...
declare namespace SteamStructs {`;
  ...
  const getReturnTypeFromCallResultJson = callresult => {
    if (!callresult || !callresult.hasOwnProperty('fields') || !Array.isArray(callresult.fields)) {
      return null;
    }
    const returnType = {};
    for (let i = 0; i < callresult.fields.length; i++) {
      const field = callresult.fields[i];
      if (!field || !field.hasOwnProperty('fieldname') || !field.hasOwnProperty('fieldtype')) {
  ...
  for (const SteamCallResultName of SteamCallResultNames) {
    const callResult = steamApiJson.callback_structs.find(cb => cb?.struct === SteamCallResultName);
    ...
    const callResultName = callResultStructName.substr(0, callResultStructName.length - 2);
  ...
  for (const SteamCallBackName of SteamCallBackNames) {
    const callBack = steamApiJson.callback_structs.find(cb => cb?.struct === SteamCallBackName);
    ...
    const callBackName = callBackStructName.substr(0, callBackStructName.length - 2);

    const callbackAltName = callBack.struct.substr(0, callBackStructName.length - 8);
    const methodsWithCallBack = steamApiJson.interfaces
      .filter(i => i?.methods?.length)
      .flatMap(i => i.methods)
      .filter(method => method?.callback === callBack.struct || method?.methodname === callbackAltName);
